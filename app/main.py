...
def select_server():
    ...
    if server_id:
        ...
        flash(
            f"Could not find server '{server_name}'. Please check the name and ensure the bot is a member of that server."
        )
        return redirect(url_for("index"))
...
def server_page(server_id):
    ...
    future = asyncio.run_coroutine_threadsafe(
        fetch_channels_from_guild(server_id), bot.loop
    )
...
def upload_handler():
    ...
    if is_folder_upload:
        ...
        folder_name = (
            custom_folder_name or os.path.dirname(files[0].filename) or "upload"
        )
        ...
        for file in files:
            ...
            temp_file_path = os.path.join(
                DATA_DIRECTORY, f"{uuid.uuid4()}{os.path.splitext(file.filename)[1]}"
            )
            file.save(temp_file_path)

            # Use the unified helper for processing
            processed_chunks, processed_basenames = process_and_chunk_file(
                temp_file_path, secure_upload
            )
            ...
            for part in path_parts[:-1]:
                current_level = current_level.setdefault(
                    part, {"type": "directory", "children": {}}
                )["children"]
            current_level[path_parts[-1]] = {
                "type": "file",
                "chunks": processed_basenames,
            }
        ...
        final_metadata = {
            ...
            "total_size": sum(
                os.path.getsize(p) for p in all_chunk_paths
            ),  # calc size of all chunks
            "tree": folder_tree.get(folder_name, {}).get("children", folder_tree),
        }

        try:
            future = asyncio.run_coroutine_threadsafe(
                upload_folder(final_metadata, all_chunk_paths, server_id, channel_name),
                bot.loop,
            )
            future.result()
            ...
            return jsonify(
                {"status": "error", "message": f"Folder upload failed: {str(e)}"}
            ), 500

        return jsonify(
            {"status": "success", "message": f"Folder '{folder_name}' uploaded."}
        )
        ...
        for file in files:
            ...
            try:
                logger.info(
                    f"Uploading file '{file.filename}' to channel '{channel_name}'..."
                )
                future = asyncio.run_coroutine_threadsafe(
                    upload_single_file(
                        temp_file_path,
                ...
                if os.path.exists(temp_file_path):
                    os.remove(temp_file_path)
        return jsonify(
            {"status": "success", "message": f"{len(files)} files uploaded."}
        )
...
def list_files_route():
    ...
    future = asyncio.run_coroutine_threadsafe(
        fetch_files_from_channel(server_id, channel_name), bot.loop
    )
    files = future.result()
...
def download_route():
    ...
    logger.info(
        f"Download for '{filename}' from server '{server_id}' in channel '{channel_name}'"
    )
    future = asyncio.run_coroutine_threadsafe(
        download_from_discord(server_id, channel_name, filename), bot.loop
    )
...
def delete_route():
    ...
    logger.info(
        f"Delete request for '{filename}' from server '{server_id}' in channel '{channel_name}'"
    )

    try:
        future = asyncio.run_coroutine_threadsafe(
            delete_from_discord(server_id, channel_name, filename), bot.loop
        )
        ...
        if success:
            ...
            return jsonify(
                {"status": "error", "message": f"Failed to delete '{filename}'."}
            ), 400
    except Exception as e:

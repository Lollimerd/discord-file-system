...
async def upload_chunk_concurrently(semaphore, channel, chunk_path, index, total):
    ...
    async with semaphore:
        try:
            ...
            logger.info(f"Uploading chunk {index}/{total}: {filename}")
            await channel.send(file=discord.File(chunk_path, filename=filename))
        except Exception as e:
            logger.error(f"Failed to upload chunk '{chunk_path}': {e}")
            raise e
        finally:
...
async def upload_single_file(file_path, original_filename, server_id, channel_name, secure):
    ...
    try:
        ...
        logger.info(f"Starting concurrent upload for {total_chunks} chunks...")
        
        for i, chunk_path in enumerate(chunk_paths):
            tasks.append(upload_chunk_concurrently(semaphore, channel, chunk_path, i+1, total_chunks))
...
async def upload_folder(metadata_obj, chunk_paths, server_id, channel_name):
    ...
    for i, chunk_path in enumerate(chunk_paths):
         # Note: Discord replaces spaces with underscores, but we handle that in download.
         # For consistency with single file upload, we can just use the basename.
         # But the original code replaced spaces. We should probably keep using the helper which sends exact filename.
         # Actually, the helper sends exact filename. The original code did .replace(' ', '_').
         # Let's check upload_chunk_concurrently implementation... it uses os.path.basename(chunk_path).
         # Chunks are usually UUID based or part-based, spaces in folder names might be an issue?
         # Chunk names are generated by process_and_chunk_file: "{base}_part_{i}".
         # If base has spaces, chunk name has spaces.
         # The helper function upload_chunk_concurrently does NOT replace spaces.
         # However, Discord DOES replace them on the server side.
         # The download logic handles this fallback.
         # So we can safely use the same helper function.
         tasks.append(upload_chunk_concurrently(semaphore, channel, chunk_path, i+1, total_chunks))
